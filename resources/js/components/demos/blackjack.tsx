...
const CARD_BACK_SRC = '/game-assets/blackjack/SteamPunk%20Card%20Deck/1-Card%20Sets/Steampunk%20Card_Back-Simple%20Border.png';
...
const CHIPS_BASE = '/game-assets/blackjack/SteamPunk%20Card%20Deck/2-Individual%20Images/3-Other';
...
export function BlackjackDemo() {
    ...
    function dealCards() {
        ...
        if (playerTotal === 21) {
            ...
            setTimeout(() => endPlayerTurnWithHands(player, dealer, remainingDeck), 0);
        }
    }
    ...
    function endPlayerTurnWithHands(playerCards: Card[], initialDealer: Card[], remainingDeck: Card[]) {
        setPhase('dealer-turn');
        const revealed: Card[] = [initialDealer[0], { ...initialDealer[1], faceUp: true }];
    ...
    return (
        <div className="relative flex min-h-screen flex-col items-center justify-center gap-6 bg-[#0f6b0f] p-8">
            ...
            <div className="relative w-full max-w-2xl rounded-3xl p-6" style={{ minHeight: '380px' }}>
                {/* Deck: stacked card backs, neat stack (deal from here) */}
                <div className="absolute top-6 left-0 h-[9.5rem] w-[6.75rem]">
                    {[0, 1, 2, 3, 4, 5].map((i) => (
                        <div
                            key={i}
                        ...
                        >
                            <img src={CARD_BACK_SRC} alt="" className="h-full w-full object-cover object-top" />
                        </div>
                    ))}
                </div>
                ...
                <div className="mb-6 min-h-[120px] rounded-xl border-2 border-amber-100/80 bg-black/20 p-4">
                    <p className="mb-2 text-center text-sm font-medium text-amber-100/90">
                        Dealer
                        {phase !== 'betting' && dealerHand.some((c) => c.faceUp) && <span className="ml-2 text-white">({dealerTotal})</span>}
                    </p>
                    <div className="flex min-h-[9rem] flex-wrap justify-center gap-3">
                        {dealerHand.map((card, i) => (
                            ...
                            >
                                {card.faceUp ? (
                                    <div className="relative h-full w-full" role="img" aria-label={cardLabel(card.id)}>
                                        <img src={CARD_FACE_BG} alt="" className="absolute inset-0 z-0 h-full w-full object-cover" />
                                        <img src={getSymbolSrc(card.id)} alt="" className="absolute inset-0 z-[1] h-full w-full object-contain p-2" />
                                        <img
                                            src={getDesignationSrc(card.id)}
                                            alt=""
                                            className="absolute top-0 left-0 z-[2] h-[36%] w-auto max-w-[48%] object-contain object-left-top"
                                        />
                                    </div>
                                ) : (
                                    <img src={CARD_BACK_SRC} alt="" className="h-full w-full object-cover object-top" />
                ...
                {/* Message strip */}
                <div className="mb-4 min-h-[2rem] text-center">{message && <p className="text-lg font-semibold text-amber-200">{message}</p>}</div>

                {/* Center: bet amount */}
                <div className="mb-6 flex min-h-[2.5rem] items-center justify-center">
                    {bet > 0 && <span className="rounded-lg bg-black/30 px-5 py-2 text-lg font-semibold text-amber-200">Bet: ${bet}</span>}
                </div>
                ...
                <div className="min-h-[120px] rounded-xl border-2 border-amber-100/80 bg-black/20 p-4">
                    <p className="mb-2 text-center text-sm font-medium text-amber-100/90">
                        Player
                        {playerHand.length > 0 && <span className="ml-2 text-white">({playerTotal})</span>}
                    </p>
                    <div className="flex min-h-[9rem] flex-wrap justify-center gap-3">
            ...
            <div className="fixed right-8 bottom-8 flex flex-col items-end gap-4">
                ...
                <div className="flex items-end gap-4">
                    {CHIP_OPTIONS.map((chip) => (
                        ...
                        >
                            <img src={chip.src} alt={`$${chip.value} chip`} className="h-14 w-14 object-contain drop-shadow-lg md:h-16 md:w-16" />
                            <span className="mt-1 text-xs font-medium text-amber-100/90">${chip.value}</span>
                        </button>
                    ))}
                </div>
            </div>
