...
export function KanbanDemo() {
    ...
    const updateColumnTitle = useCallback((columnId: string, title: string) => {
        setColumns((prev) => prev.map((c) => (c.id === columnId ? { ...c, title: title.trim() || c.title } : c)));
    ...
    const updateCard = useCallback((columnId: string, cardId: string, text: string) => {
        ...
        setCardsByColumn((prev) => ({
            ...prev,
            [columnId]: (prev[columnId] ?? []).map((c) => (c.id === cardId ? { ...c, text: trimmed } : c)),
        }));
        setEditingCard(null);
        setEditingCardText('');
    }, []);
        ...
        <div className="flex min-h-screen flex-col bg-neutral-950 p-6 pt-12 md:p-8 md:pt-16">
            <header className="mb-8">
                <LampContainer className="py-6 md:py-8">
                    <h1 className="text-2xl font-semibold tracking-tight text-white md:text-3xl">Kanban</h1>
                    <p className="mt-1 text-sm text-neutral-400">Drag cards between lists · Click the pencil icon to edit</p>
                </LampContainer>
            </header>
            <div className="flex flex-1 gap-5 overflow-x-auto pb-4">
                {columns.map((col) => (
                    <div
                        key={col.id}
                        className={cn(
                            'flex max-w-[272px] min-w-[272px] flex-shrink-0 flex-col rounded-xl border border-neutral-700/80 bg-neutral-900/80 p-4 shadow-lg transition-colors',
                            'border-t-4',
                        ...
                        <div className="mb-2 flex items-center justify-between gap-2">
                            {editingColumnId === col.id ? (
                                ...
                                <button
                                    ...
                                    className="min-w-0 flex-1 truncate text-left text-sm font-semibold tracking-wide text-neutral-200 uppercase hover:text-white"
                                >
                                    {col.title}
                                </button>
                            )}
                                    ...
                                    {editingCard?.cardId === card.id && editingCard?.columnId === col.id ? (
                                        ...
                                        >
                                            <textarea
                                                ...
                                                rows={2}
                                                className="w-full resize-none rounded-md border border-neutral-600 bg-neutral-800 px-3 py-2 text-sm text-white focus:border-brand focus:ring-1 focus:ring-brand focus:outline-none"
                                                autoComplete="off"
                                            />
                            ...
                            {addingCardColumnId === col.id ? (
                                ...
                                >
                                    <textarea
                                        ...
                                        rows={2}
                                        className="w-full resize-none rounded-md border border-neutral-600 bg-neutral-800 px-3 py-2 text-sm text-white placeholder-neutral-500 focus:border-brand focus:ring-1 focus:ring-brand focus:outline-none"
                                        autoComplete="off"
                                    />
                                    <div className="flex items-center gap-2">
                                        <button
                ...
                <div className="max-w-[260px] min-w-[260px] flex-shrink-0">
                    {addingList ? (
                        <form
                            onSubmit={(e) => {
                                e.preventDefault();
                        ...
                        >
                            ...
                            />
                            <div className="flex items-center gap-2">
                                <button type="submit" className="rounded-md bg-brand px-3 py-1.5 text-sm font-medium text-white hover:bg-brand-mid">
                                    Add list
                                </button>
                                <button
                                    type="button"
