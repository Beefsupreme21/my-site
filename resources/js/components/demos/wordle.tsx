        ...
        <div className="flex min-h-screen flex-col items-center justify-center p-8 pt-16">
            <div className="w-full max-w-sm">
                ...
                <p className="mb-6 text-center text-sm text-neutral-400">
                    Guess the 5-letter word in 6 tries. Type 5 letters then press{' '}
                    <kbd className="rounded bg-neutral-700 px-1.5 py-0.5 text-xs">Enter</kbd> to submit.
                </p>
                    ...
                    {Array.from({ length: ROWS }).map((_, rowIndex) => (
                        <div key={rowIndex} className="flex justify-center gap-1.5">
                            {Array.from({ length: COLS }).map((_, colIndex) => {
                                ...
                                const letter = isCurrentRow ? (currentGuess[colIndex] ?? '') : (guesses[rowIndex]?.[colIndex] ?? '');
                                const feedback = rowIndex < guesses.length ? getFeedback(guesses[rowIndex], answer)[colIndex] : null;
                                return (
                                    <div
                                        key={colIndex}
                                        className={cn(
                ...
                {gameOver === 'win' && (
                    <div className="mb-4 text-center">
                        ...
                        <button
                            type="button"
                            onClick={reset}
                            className="mt-2 rounded-lg bg-brand px-4 py-2 text-sm font-medium text-white hover:bg-brand-mid"
                        >
                            Play again
                        </button>
                    </div>
                ...
                {gameOver === 'lose' && (
                    <div className="mb-4 text-center">
                        ...
                        <p className="text-xl font-bold text-white uppercase">{answer}</p>
                        <button
                            type="button"
                            onClick={reset}
                            className="mt-2 rounded-lg bg-brand px-4 py-2 text-sm font-medium text-white hover:bg-brand-mid"
